      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-slate-950">
        <div className="w-full max-w-md space-y-8 animate-fade-in">
          ...
          <p className="text-center text-slate-500 text-sm">
             {isRegistering ? (
               <>Already have an account? <button onClick={() => setIsRegistering(false)} className="text-emerald-400 hover:underline font-medium py-1 px-1">Sign In</button></>
             ) : (
               <>{t.noAccount} <button onClick={() => setIsRegistering(true)} className="text-emerald-400 hover:underline font-medium py-1 px-1">{t.signUp}</button></>
             )}
          </p>
